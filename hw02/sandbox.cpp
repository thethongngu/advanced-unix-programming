...
#define	OLD_FUNC(name)	static void* old_##name = NULL;
...
#define	HOOK(ret, name, ...)	\
	...
	ret name(__VA_ARGS__) { \
	    if (old_##name == NULL) { \
	        ...
	        if (handle != NULL) old_##name = dlsym(handle, #name); \
	    } \
        if (old_##name != NULL) return old_##name(__VA_ARGS__);\
	}
...
//        fprintf(stderr, "%s\n", pathname);
    ...
    FILE *fopen(const char *pathname, const char *mode, string x) {
        if(old_chmod == nullptr) {
            void *handle = dlopen(LIBC, RTLD_LAZY);
        ...
        if (!check_valid(pathname, x)) {
            ...
            if(old_fopen != nullptr) old_fopen(pathname, mode);
            return 0;
        }
    }
    ...
    int open(const char *pathname, int flags, mode_t mode, string x) {
        ...
        if (!check_valid(pathname, x)) {
            ...
            if(old_open != nullptr) old_open(pathname, mode);
            return 0;
        }
    }
    ...
    //-------------------------------------------------
    static int (*old___xstat)(const char *, struct stat) = nullptr;
    int __xstat(const char *path, struct stat *buf, string x) {
        if(old_chmod == nullptr) {
            void *handle = dlopen(LIBC, RTLD_LAZY);
            if(handle != nullptr) old___xstat = reinterpret_cast<int (*)(const char *, struct stat)>(dlsym(handle, "__xstat"));
        }
        if (!check_valid(path, x)) {
            fprintf(stderr, "[sandbox]: __xstat: access to %s is not allowed", path);
            return NULL;
        } else {
            if(old___xstat != nullptr) old___xstat(path);
            return 0;
        }
    }
    ...
    int rename(const char *oldname, const char *newname, string x) {
        if(old_chmod == nullptr) {
            void *handle = dlopen(LIBC, RTLD_LAZY);
    ...
    int openat(int dirfd, const char *pathname, int flags, mode_t mode, string x) {
        ...
        if (!check_valid(pathname, x)) {
            ...
            if(old_openat != nullptr) old_openat(pathname);
            return 0;
        }
    }
